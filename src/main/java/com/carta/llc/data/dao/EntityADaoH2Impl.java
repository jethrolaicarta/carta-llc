 ...
 */
@Repository
public class EntityADaoH2Impl implements EntityADao {

	private static final String ENTITY_A_TABLE_NAME = "entity_a";
	final private Logger logger = LoggerFactory.getLogger(EntityADaoH2Impl.class);
	...
	public List<EntityA> getAll() {
		List<EntityA> entityAs = jdbcTemplate.query(String.format("SELECT * FROM %s", ENTITY_A_TABLE_NAME),
				new Object[] {}, (rs, rowNum) -> new EntityA(rs.getString("uuid"), rs.getString("status"),
						rs.getDate("created_date"), rs.getString("detail"), rs.getDate("updated_date")));
